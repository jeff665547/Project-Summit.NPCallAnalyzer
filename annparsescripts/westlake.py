...
westlake_aruco_id_map = siconfig.get_chip_arucoid(jchip)


def polyt_chrome_ann():
        
    westlake_chip = np.ones((probe_num_r, probe_num_c), dtype = int) - 2
    
    # i = '47'
    for i in list(westlake_aruco_id_map.keys()):
        marker_pos_c = (np.array(westlake_aruco_id_map[i]) * marker_stride_c_probe)[0] # col  (horizontal)
        marker_pos_r = (np.array(westlake_aruco_id_map[i]) * marker_stride_r_probe)[1] # row  (vertical)
        ...
        westlake_chip[code_pos_r:code_pos_r + code.shape[0], code_pos_c:code_pos_c + code.shape[1]] += code
        westlake_chip[marker_pos_r:marker_pos_r + edge.shape[0], marker_pos_c:marker_pos_c + edge.shape[1]] += edge
    
    westlake_ann = pd.DataFrame()
    
    x = list(np.where(westlake_chip == 1)[1])  # horizontal
    y = list(np.where(westlake_chip == 1)[0])  # vertical
    ref = ['Chrome']*len(np.where(westlake_chip == 1)[1])
    x.extend(list(np.where(westlake_chip == 0)[1]))
    y.extend(list(np.where(westlake_chip == 0)[0]))
    ref.extend(['PolyT']*len(np.where(westlake_chip == 0)[1]))
    
    westlake_ann['x'] = x
    westlake_ann['y'] = y
    westlake_ann['ref'] = ref
    del westlake_chip
...
def AM1_AM3_AM5_ann():
    ...
    westlake_AM_ann = pd.DataFrame()
    westlake_AM_ann['x'] = col # horizontal
    westlake_AM_ann['y'] = row # vertical
    westlake_AM_ann['ref'] = np.array(ref)

    return westlake_AM_ann
...
def probe_ann():
    ...
    with open(sipath.settings_jfile) as f:
        ...
        annot = pd.read_csv(info["path"], usecols = columns, sep = "\t")
        annot.probe_id = annot.probe_id.fillna("NULL")
